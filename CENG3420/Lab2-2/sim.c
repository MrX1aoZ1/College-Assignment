...
void handle_slli(unsigned int cur_inst) {
     ...
     */
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    int imm5 = sext(MASK24_20(cur_inst), 5);
...
void handle_srai(unsigned int cur_inst) {
    ...
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    int imm5 = sext(MASK24_20(cur_inst), 5);
    int rs1Current = CURRENT_LATCHES.REGS[rs1];
    NEXT_LATCHES.REGS[rd] = sext(rs1Current >> imm5, 32 - imm5);
}
...
void handle_lui(unsigned int cur_inst) {
    ...
    unsigned int imm20 = sext(MASK31_12(cur_inst), 20);
...
void handle_sra(unsigned int cur_inst) {
    ...
    unsigned int rd = MASK11_7(cur_inst),
        ...
        rs2 = MASK24_20(cur_inst);
    int rs1Current = CURRENT_LATCHES.REGS[rs1];
    NEXT_LATCHES.REGS[rd] = sext(rs1Current >> CURRENT_LATCHES.REGS[rs2], 32 - CURRENT_LATCHES.REGS[rs2]);
...
void handle_jalr(unsigned int cur_inst) {
    ...
    NEXT_LATCHES.REGS[rd] = CURRENT_LATCHES.PC + 4;                 // rd = PC + 4
    NEXT_LATCHES.PC = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1];  // PC = rs1 + imm
}
...
void handle_jal(unsigned int cur_inst) {
     ...
     */
    unsigned int rd = MASK11_7(cur_inst);
    int imm21 = (MASK31(cur_inst) << 20) + \
        (MASK30_21(cur_inst) << 1) + \
        (MASK20(cur_inst) << 11) + \
        (MASK19_12(cur_inst) << 12);
    NEXT_LATCHES.REGS[rd] = CURRENT_LATCHES.PC + 4;                 // rd = PC + 4
    NEXT_LATCHES.PC = sext(imm21, 21) + CURRENT_LATCHES.PC;         // PC += imm
...
void handle_beq(unsigned int cur_inst) {
    ...
    int imm13 = (MASK31(cur_inst) << 12) + \
            (MASK7(cur_inst) << 11) + \
            (MASK30_25(cur_inst) << 5) + \
            (MASK11_8(cur_inst) << 1);
    if (CURRENT_LATCHES.REGS[rs1] == CURRENT_LATCHES.REGS[rs2])
        NEXT_LATCHES.PC = sext(imm13, 13) + CURRENT_LATCHES.PC;
}
...
void handle_lh(unsigned int cur_inst) {
    ...
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    int imm12 = MASK31_20(cur_inst);
    unsigned int address = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1];
    int value = ((int)MEMORY[address]) + \
        ((int)MEMORY[address + 1] << 8);
...
void handle_lw(unsigned int cur_inst) {
    ...
    int value = ((int)MEMORY[address]) + \
        ((int)MEMORY[address + 1] << 8) + \
        ((int)MEMORY[address + 2] << 16) + \
        ((int)MEMORY[address + 3] << 24);
...
void handle_sb(unsigned int cur_inst) {
     ...
     */
    unsigned int rs1 = MASK19_15(cur_inst), rs2 = MASK24_20(cur_inst);
    int imm12 = (sext(MASK31_25(cur_inst), 7) << 5 ) + MASK11_7(cur_inst);
    unsigned int address = imm12 + CURRENT_LATCHES.REGS[rs1]; 
    MEMORY[address] = CURRENT_LATCHES.REGS[rs2];
}
...
void handle_sh(unsigned int cur_inst) {
    ...
    int imm12 = (sext(MASK31_25(cur_inst), 7) << 5 ) + MASK11_7(cur_inst);
    unsigned int address = imm12 + CURRENT_LATCHES.REGS[rs1]; 
    MEMORY[address] = MASK7_0(CURRENT_LATCHES.REGS[rs2]);
    MEMORY[address + 1] = MASK15_8(CURRENT_LATCHES.REGS[rs2]);
}
...
void handle_sw(unsigned int cur_inst) {
    ...
    MEMORY[address] = MASK7_0(CURRENT_LATCHES.REGS[rs2]);
    MEMORY[address + 1] = MASK15_8(CURRENT_LATCHES.REGS[rs2]);
    MEMORY[address + 2] = MASK23_16(CURRENT_LATCHES.REGS[rs2]);
    MEMORY[address + 3] = MASK31_24(CURRENT_LATCHES.REGS[rs2]);
}
...
void handle_instruction() {
    ...
    switch(opcode) {
        ...
        case (0b0110011):
            /*
             * Interger Register-Register Instruction
             */ 
            switch (funct3)
            {
                case 0:
                    if (MASK31_25(cur_inst) == 0x00)
                        handle_add(cur_inst);
                    else if (MASK31_25(cur_inst) == 0x20)
                        handle_sub(cur_inst);
                    break;
                case 4:
                    handle_xor(cur_inst);
