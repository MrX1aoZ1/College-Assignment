...
int inst_to_binary(
    ...
    if (is_opcode(opcode) == ADDI) {
        ...
        /* Lab2-1 assignment */
        binary += 0b0010011;                                            // opcode [6:0]
        binary += (reg_to_num(arg1, line_no) << 7);                     // rd [11:7] 
        binary += (0x1 << 12);                                          // func3 [14:12]
        binary += (reg_to_num(arg2, line_no) << 15);                    // rs1 [19:15] 
        binary += (MASK11_0(validate_imm(arg3, 12, line_no)) << 20);    // imm[4:0] [24:20]
        binary += (0x00 << 25);                                         // imm[11:5] [31:25]
    } else if (is_opcode(opcode) == XORI) {
        /* Lab2-1 assignment */
        binary += 0b0010011;                                            // opcode [6:0]
        binary += (reg_to_num(arg1, line_no) << 7);                     // rd [11:7] 
        binary += (0x4 << 12);                                          // func3 [14:12]
        binary += (reg_to_num(arg2, line_no) << 15);                    // rs1 [19:15] 
        binary += (MASK11_0(validate_imm(arg3, 12, line_no)) << 20);    // imm[11:0] [31:20]
        ...
        binary += 0b0010011;                                            // opcode [6:0]
        binary += (reg_to_num(arg1, line_no) << 7);                     // rd [11:7] 
        binary += (0x5 << 12);                                          // func3 [14:12]
        binary += (reg_to_num(arg2, line_no) << 15);                    // rs1 [19:15] 
        binary += (lower5bit(arg3, line_no) << 20);                     // imm[4:0] [24:20]
        binary += (0x00 << 25);                                         // imm[11:5] [31:25]
        ...
        binary += (reg_to_num(arg2, line_no) << 15);                    // rs1 [19:15] 
        binary += (lower5bit(arg3, line_no) << 20);                     // imm[4:0] [24:20]                                     
        binary += (0x20 << 25);                                         // imm[11:5] [31:25]
    } else if (is_opcode(opcode) == ORI) {
        /* Lab2-1 assignment */
        binary += 0b0010011;                                            // opcode [6:0]
        binary += (reg_to_num(arg1, line_no) << 7);                     // rd [11:7] 
        binary += (0x6 << 12);                                          // func3 [14:12]
        binary += (reg_to_num(arg2, line_no) << 15);                    // rs1 [19:15] 
        binary += (MASK11_0(validate_imm(arg3, 12, line_no)) << 20);    // imm[11:0] [31:20]
    ...
    // Integer Register-Register Operations
        ...
        /* Lab2-1 assignment */
        binary = 0b0110011;                             // opcode   [6:0]
        binary += (reg_to_num(arg1, line_no) << 7);     // rd       [11:7] 
        binary += (0x0 << 12);                          // func3    [14:12]
        binary += (reg_to_num(arg2, line_no) << 15);    // rs1      [19:15] 
        binary += (reg_to_num(arg3, line_no) << 20);    // rs2      [24:20]
        binary += (0x20 << 25);                         // func7    [31:25]
        ...
        binary += (0x1 << 12);                          // func3    [14:12]
        binary += (reg_to_num(arg2, line_no) << 15);    // rs1      [19:15] 
        binary += (reg_to_num(arg3, line_no) << 20);    // rs2      [20:24]
        binary += (0x00 << 25);                         // func7    [25:29]        
    ...
    // Unconditional Jumps
         ...
         */
        binary = 0b1100111;                                 // opcode       [6:0]
        binary += (reg_to_num(arg1, line_no) << 7);         // rd           [11:7]
        binary += (0x0 << 12);                              // func3        [14:12]
        struct_regs_indirect_addr* ret = parse_regs_indirect_addr(arg2, line_no);
        binary += (reg_to_num(ret->reg, line_no) << 15);    // rs1          [19:15]
        binary += (MASK11_0(ret->imm) << 20);               // imm[11:0]    [31:20] // relative position of rs1
         ...
         */
        binary = 0b1101111;                                 // opcode       [6:0]
        binary += (reg_to_num(arg1, line_no) << 7);         // rd           [11:7]
        int val = handle_label_or_imm(line_no, arg2, label_table, number_of_labels);
        int offset = val - addr;                            // imm = jumpaddr - currentPC
        binary += ((offset & 0xFF000) >> 12 << 12);         // imm[19:12]   [19:12]
        binary += ((offset & 0x800) >> 11 << 20);           // imm[11]      [20]
        binary += ((offset & 0x7FE) >> 1 << 21);            // imm[10:1]    [30:21]
        binary += ((offset & 0x100000) >> 20 << 31);        // imm[20]      [31]
    }
    ...
    // Conditional Branches
        ...
        /* Lab2-1 assignment */
        binary = 0b1100011;                             // opcode       [6:0]
        binary += (reg_to_num(arg1, line_no) << 15);    // rs1          [19:15]
        binary += (reg_to_num(arg2, line_no) << 20);    // rs2          [24:20]
        binary += (0x1 << 12);                          // funct3       [14:12]
        int val = label_to_num(line_no, arg3, 12, label_table, number_of_labels);
        int offset = val - addr;
        binary += ((offset & 0x800) >> 11 << 7);        // imm[11]      [7]          
        binary += ((offset & 0x1E) >> 1 << 8);          // imm[4:1]     [11:8]
        binary += ((offset & 0x7E0) >> 5 << 25);        // imm[10:5]    [30:25]
        binary += ((offset & 0x1000) << 19);            // imm[12]      [31] 
    } else if (is_opcode(opcode) == BLT) {
        /* Lab2-1 assignment */
        binary = 0b1100011;                             // opcode       [6:0]
        binary += (reg_to_num(arg1, line_no) << 15);    // rs1          [19:15]
        binary += (reg_to_num(arg2, line_no) << 20);    // rs2          [24:20]
        binary += (0x4 << 12);                          // funct3       [14:12]
        int val = label_to_num(line_no, arg3, 12, label_table, number_of_labels);
        int offset = val - addr;
        binary += ((offset & 0x800) >> 11 << 7);        // imm[11]      [7]          
        binary += ((offset & 0x1E) >> 1 << 8);          // imm[4:1]     [11:8]
    ...
    // Load and Store Instructions
        ...
        /* Lab2-1 assignment */
        binary = 0b0000011;                                 // opcode       [6:0]
        binary += (reg_to_num(arg1, line_no) << 7);         // rd           [7:11]
        binary += (0x1 << 12);                              // funct3       [14:12]
        struct_regs_indirect_addr* ret = parse_regs_indirect_addr(arg2, line_no);
        binary += (reg_to_num(ret->reg, line_no) << 15);    // rs1          [19:15]
        binary += (MASK11_0(ret->imm  << 20));              // imm[11:0]    [31:20]
    } else if (is_opcode(opcode) == LW) {
        /* Lab2-1 assignment */
        binary = 0b0000011;                                 // opcode       [6:0]
        binary += (reg_to_num(arg1, line_no) << 7);         // rd           [7:11]
        binary += (0x2 << 12);                              // funct3       [14:12]
        struct_regs_indirect_addr* ret = parse_regs_indirect_addr(arg2, line_no);
        binary += (reg_to_num(ret->reg, line_no) << 15);    // rs1          [19:15]
        binary += (MASK11_0(ret->imm  << 20));              // imm[11:0]    [31:20]
    } else if (is_opcode(opcode) == SB) {
        /* Lab2-1 assignment */
        binary = 0b0100011;                                 // opcode       [6:0]
        struct_regs_indirect_addr* ret = parse_regs_indirect_addr(arg2, line_no);
        binary += ((ret->imm & 0x1F) << 7);                 // imm[4:0]     [7:11]
        binary += (0x0 << 12);                              // funct3       [14:12]
        binary += (reg_to_num(ret->reg, line_no) << 15);    // rs1          [19:15]
        binary += (reg_to_num(arg1,line_no) << 20);         // rs2          [24:20]
        binary += ((ret->imm & 0xFE0) >> 5 << 25);          // imm[11:5]    [31:25]
