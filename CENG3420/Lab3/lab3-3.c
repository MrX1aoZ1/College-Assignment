...
void eval_bus_drivers() {
    ...
    /* input of GateMAR */
    // value_of_GateMAR = ?;
    if (get_MARMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 1)
        value_of_GateMAR = logic_shift_20_function_unit(mask_val(CURRENT_LATCHES.IR, 31, 12));
    else if (get_MARMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 0)
    ...
    value_of_shift_function_unit = shift_function_unit(
        ...
        rs2_mux(
            ...
            sext_unit(mask_val(CURRENT_LATCHES.IR, 24, 20), 5)
        )
    );
     ...
     */
    /* input of GateRS2 */
    value_of_GateRS2 = rs2_en(get_RS2En(CURRENT_LATCHES.MICROINSTRUCTION), CURRENT_LATCHES.REGS[mask_val(CURRENT_LATCHES.IR, 24, 20)]);
